...
function buildPathData(path: readonly [number, number, number][]): PathData {
...
function posAtDist(
  ...
  if (dist <= 0) return { x: path[0][0], y: path[0][1], z: path[0][2] }
  ...
  for (let i = 0; i < pd.segLengths.length; i++) {
    if (dist < pd.cumLengths[i + 1]) {
      ...
      return {
        ...
        y: path[i][1] + (path[i + 1][1] - path[i][1]) * localT,
        z: path[i][2] + (path[i + 1][2] - path[i][2]) * localT,
      }
    }
  }
  const last = path[path.length - 1]
...
export function RailCameraWithHeightControl({
