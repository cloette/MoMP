...
// Ordered left→right (x=-6 is leftmost from camera's view).
// "First on the right" = x=6 → /art-hall, per spec.
const FRONT_DOORS: { x: number; label: string; route: string }[] = [
  { x: 6, label: 'Wish Room', route: '/wish-room' },
  { x: 2, label: 'Library',   route: '/library'   },
  { x:  -2, label: 'Exhibit A', route: '/room-a'    },
...
function DoorPortal({
  ...
  return (
    <group position={position} rotation={[0, rotationY, 0]}>
      ...
      {/* Label — screenspace so it always reads correctly regardless of rotationY */}
      { label && (
...
// ── Path layer helper ────────────────────────────────────────────────────────
// Renders one horizontal plane.  args = [worldX_size, worldZ_size].
function PathLayer({
